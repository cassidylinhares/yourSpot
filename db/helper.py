...
class Connection:
    ...
    def getFavs(self, userID): # triple table join; view 1
        self.cur.execute(
            '''select s.id, r.name, u.username, s.restaurant_id, s.user_rating, s.review, s.date_reviewed from 
            ((user_saved_restaurants as s join restaurant as r on s.restaurant_id  = r.id) 
            join user as u on s.user_id = u.id) 
            where u.id = %s''', [userID])
        return self.cur.fetchall()

    def rmFromFavs(self, id):
        self.cur.execute(
            'delete from user_saved_restaurants where id = %s',
            [id])
        self.con.commit()
